...
def _footer(canvas, doc):
    ...
    canvas.drawString(18 * mm, 12 * mm, "PRISM \u00b7 CONFIDENTIAL \u2014 FOR DILIGENCE USE ONLY")
...
def render_report_pdf(session_id: str, report: Dict[str, Any], out_path: str) -> str:
    ...
    story.append(Paragraph(f"Generated {gen_date} · Ref. {session_id[:8]}", styles["subtitle"]))

    score = report['readiness_score']
    score_color = VERIFIED if score >= 75 else (AMBER if score >= 50 else REDINK)
    score_style = ParagraphStyle("score_colored", parent=styles["score"], textColor=score_color)
    score_tbl = Table(
        [[
    ...
    for row in report["matrix"]:
        ...
        def _cell(k: str) -> str:
            v = docs.get(k)
            val = (v or {}).get("value", "").strip() if v else ""
